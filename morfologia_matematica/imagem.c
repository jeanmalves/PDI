...
/*----------------------------------------------------------------------------*/
/** Redimensionamento por vizinho mais pr�ximo. Simples e barato. Eu nem estou
 * pegando o vizinho mais pr�ximo de verdade, estou truncando para baixo - o
 * resultado � quase o mesmo.
 *
 * Par�metros: Imagem* in: imagem de entrada.
 *             Imagem* out: imagem de sa�da. Deve ter o mesmo n�mero de canais
...
/*----------------------------------------------------------------------------*/
/** Redimensionamento com interpola��o bilinear.
 *
 * Par�metros: Imagem* in: imagem de entrada.
 *             Imagem* out: imagem de sa�da. Deve ter o mesmo n�mero de canais
 *               da imagem de entrada.
...
void redimensionaBilinear (Imagem* in, Imagem* out)
{
    ...
    for (row = 0; row < out->altura; row++)
    {
        // Calcula os par�metros verticais.
        y = row*vscale-0.5f;
        floor_y = floorf (y);
        t = ((int) floor_y);
        ...
        for (col = 0; col < out->largura; col++)
        {
            // Calcula os par�metros horizontais.
            x = col*hscale-0.5f;
            floor_x = floorf (x);
            l = ((int) floor_x);
